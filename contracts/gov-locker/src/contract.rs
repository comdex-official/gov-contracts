...
use crate::state::{
    ...
    SUPPLY, TOKENS, UNLOCKED, UNLOCKING, VTOKENS, CallType, LockingPeriodOfTokens,
};
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Lock {app_id,locking_period, calltype } => handle_lock_nft(deps, env, info, app_id, locking_period,calltype),

        ExecuteMsg::Unlock { app_id, denom ,locking_period} => handle_unlock_nft(deps, env, info, app_id, denom,locking_period),
        ...
        ExecuteMsg::Withdraw {
            ...
            amount,
            locking_period,
        } => withdraw(deps, &env, info, denom, amount,locking_period),
    }
}
...
if calltype == CallType::Lock{
    match nft {
        Some(mut token) => {
            ...
            if res.is_empty() {
                ...
                if vtokenperiod.is_empty(){
                    let _period =LockingPeriodOfTokens{
                ...
                let _period =vtokenperiod[0].clone();
        ...
        let vtoken = VTOKENS.load(deps.storage, (info.sender,&info.funds[0].denom)).unwrap();
        ...
        let vtokenperiod: Vec<&LockingPeriodOfTokens> = vtoken
                ...
                .collect();
                vtokenperiod[0].end_time = vtokenperiod[0].end_time.plus_seconds(period);
    }else {
...
pub fn handle_unlock_nft(
    ...
    if vtokenperiod[0].end_time < env.block.time
        && vtokenperiod[0].end_time.seconds() + state.unlock_period > env.block.time.seconds()
    {
        vtokenperiod[0].status = Status::Unlocking;
        // UNLOCKING.save(deps.storage, info.sender, data)
    } else if vtokenperiod[0].end_time.seconds() + state.unlock_period < env.block.time.seconds() {
        vtokenperiod[0].status = Status::Unlocked
    } else {
