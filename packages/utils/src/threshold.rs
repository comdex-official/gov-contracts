...
impl Threshold {
    ...
    pub fn validate(&self, total_weight: u128) -> Result<(), ThresholdError> {
        match self {
    ...
    pub fn to_response(&self, total_weight: u128) -> ThresholdResponse {
        match self.clone() {
...
pub fn valid_threshold(percent: &Decimal) -> Result<(), ThresholdError> {
...
/// Asserts that the 0.5 < percent <= 1.0
pub fn valid_quorum(percent: &Decimal) -> Result<(), ThresholdError> {
    if percent.is_zero() {
...
pub enum ThresholdResponse {
    ...
    AbsoluteCount { weight: u128, total_weight: u128 },
